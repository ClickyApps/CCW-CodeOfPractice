...
$(function(){
  ...
  $('.scroller .up').bind('touchend', function(){
    ...
    {
      var ScrollTop = $(Scrollable).scrollTop() - 4;
      $(Scrollable).scrollTop(ScrollTop);
      return;
    }
    
    var New = parseInt($(Scrollable).css('margin-top')) + 4;
  ...
  $('.scroller .down').bind('touchend', function(){
    ...
    var Available = 270-$(Scrollable).height();
    var New = parseInt($(Scrollable).css('margin-top')) - 4;
  ...
  $('#page-message #message-send').bind('click', function(){
    ...
    $.getJSON(Settings.Messaging + '?Callback=?', 
        {FirstName:FirstName, LastName:LastName, Email:Email, Phone:Phone, Message:Message, Welsh:Welsh}, 
      function(Response){
